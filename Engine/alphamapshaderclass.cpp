...
bool AlphaMapShaderClass::Initialize(ID3D11Device* device)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, L"data/shaders/alphamap.vs", L"data/shaders/alphamap.ps");
	if (!result) {
		return false;
	}
...
bool AlphaMapShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
