#include "SkyPlaneVolumetric.h"

SkyPlaneVolumetric::SkyPlaneVolumetric() : ModelClass()
{
	m_skyPlane = 0;
	m_shader = 0;
}
...
SkyPlaneVolumetric::~SkyPlaneVolumetric()
{
}

bool SkyPlaneVolumetric::Initialize(D3DClass* d3dClass)
{
	...
	m_shader = new VolumetricClouds;
	m_shader->setD3D(m_D3D);
	if (!m_shader->Initialize(m_D3D->GetDevice())) {
		return false;
	}

	m_shaderlight = new LightShaderClass;
	m_shaderlight->Initialize(m_D3D->GetDevice());

	return true;
}

void SkyPlaneVolumetric::Shutdown()
{
	if (m_shader) {
		m_shader->Shutdown();
		m_shader;
...
void SkyPlaneVolumetric::ShutdownSkyPlane()
{
	// Release the sky plane array.
	if (m_skyPlane) {
		delete[] m_skyPlane;
...
void SkyPlaneVolumetric::frame(CameraClass* camera, float time)
{
	m_counter += time;
	if (m_counter > 10) {
...
float SkyPlaneVolumetric::GetBrightness()
{
	return m_brightness;
}

float SkyPlaneVolumetric::GetTranslation()
{
	return m_translation;
}

bool SkyPlaneVolumetric::InitializeSkyPlane(int skyPlaneResolution, float skyPlaneWidth, float skyPlaneTop, float skyPlaneBottom, int textureRepeat)
{
...
bool SkyPlaneVolumetric::InitializeBuffers(ID3D11Device* device, int skyPlaneResolution)
{
	VertexType* vertices;
...
void SkyPlaneVolumetric::Render(CameraClass* camera)
{
	//m_shader->computeVolumetricCloudsShaders(camera);
	//return;
	...
	m_shader->computeVolumetricCloudsShaders(camera);
	//m_shader->Render(m_D3D->GetDeviceContext(), GetIndexCount());
	m_shaderlight->clearLights();
	m_shaderlight->addLights({ getLight(0) });
	std::vector<ID3D11ShaderResourceView*> textureArray;
	textureArray.push_back(m_shader->getPrevClouds());
	m_shaderlight->Render(m_D3D->GetDeviceContext(), GetIndexCount(), worldMatrix, viewMatrix, projectionMatrix, textureArray.data(), camera->GetPosition());

	m_D3D->TurnOffAlphaBlending();
}

void SkyPlaneVolumetric::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
