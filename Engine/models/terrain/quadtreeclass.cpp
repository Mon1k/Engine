...
bool QuadTreeClass::Initialize(void* terrain, int vertexCount, D3DClass* d3dClass, FrustumClass* frustum)
{
	...
	memcpy(m_vertexList, terrain, sizeof(VertexType) * vertexCount);
...
void QuadTreeClass::CreateTreeNode(NodeType* node, float positionX, float positionZ, float width)
{
	...
	m_D3D->GetDevice()->CreateBuffer(&vertexBufferDesc, &vertexData, &node->vertexBuffer);
	...
	m_D3D->GetDevice()->CreateBuffer(&indexBufferDesc, &indexData, &node->indexBuffer);
...
void QuadTreeClass::RenderNode(NodeType* node, TerrainShaderClass* shader)
{
	...
	for (i = 0; i < 4; i++) {
		if (node->nodes[i] != 0) {
			count++;
			RenderNode(node->nodes[i], shader);
		}
	}
