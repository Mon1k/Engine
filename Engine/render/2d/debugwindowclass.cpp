...
bool DebugWindowClass::Initialize(D3DClass* d3d, int screenWidth, int screenHeight, int bitmapWidth, int bitmapHeight)
{
	bool result;
...
bool DebugWindowClass::Render(int positionX, int positionY)
{
	...
	result = UpdateBuffers(m_D3D->GetDeviceContext(), positionX, positionY);
	if (!result) {
		return false;
	}
