...
bool ColorShaderClass::Initialize(ID3D11Device* device)
{
	...
	result = InitializeShader(device, L"data/shaders/color.vs", L"data/shaders/color.ps");
	if (!result) {
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	if (FAILED(result)) {
		...
		// If there was  nothing in the error message then it simply could not find the shader file itself.
			...
			MessageBox(NULL, vsFilename, L"Missing vertex shader file", MB_OK);
		}
	...
	if (FAILED(result)) {
		...
		// If there was nothing in the error message then it simply could not find the file itself.
			...
			MessageBox(NULL, psFilename, L"Missing pixel shader file", MB_OK);
		}
