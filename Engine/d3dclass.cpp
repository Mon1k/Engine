						  ...
						  float screenDepth, float screenNear)
{
	...
	stringLength = 0;
	size_t size = static_cast<size_t>(stringLength);
	error = wcstombs_s(&size, m_videoCardDescription, 128, adapterDesc.Description, 128);
	if (error != 0) {
		return false;
	}
	...
	UINT flagsDevice = D3D11_CREATE_DEVICE_DEBUG;
	// Create the swap chain, Direct3D device, and Direct3D device context.
	result = D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flagsDevice, &featureLevel, 1,
