...
bool SkyDomeShaderClass::Initialize(ID3D11Device* device)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, L"./data/shaders/skydome.vs", L"./data/shaders/skydome.ps");
	if (!result) {
		return false;
	}
...
bool SkyDomeShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
