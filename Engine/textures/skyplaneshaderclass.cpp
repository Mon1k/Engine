...
bool SkyPlaneShaderClass::Initialize(ID3D11Device* device)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, L"./data/shaders/skyplane.vs", L"./data/shaders/skyplane.ps");
	if (!result) {
		return false;
	}
...
bool SkyPlaneShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
