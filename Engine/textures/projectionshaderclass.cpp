...
bool ProjectionShaderClass::Initialize(ID3D11Device* device, WCHAR* textFileName)
{
	// Initialize the vertex and pixel shaders.
	if (!InitializeShader(device, L"./data/shaders/projection.vs", L"./data/shaders/projection.ps")) {
		return false;
	}

	m_pointView = new ViewPointClass;
	
	m_Texture = new TextureClass();
	if (!m_Texture->Initialize(device, textFileName)) {
		return false;
	}
...
bool ProjectionShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
	D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView** textureArray, D3DXVECTOR3 cameraPositio)
{
	bool result;

	D3DXMATRIX viewMatrix2, projectionMatrix2;

	m_pointView->GetViewMatrix(viewMatrix2);
	m_pointView->GetProjectionMatrix(projectionMatrix2);

	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureArray[0], m_light->GetAmbientColor(), m_light->GetDiffuseColor(), m_light->GetDirection(), viewMatrix2, projectionMatrix2, m_Texture->GetTexture());
	if (!result) {
		return false;
	}
...
bool ProjectionShaderClass::InitializeShader(ID3D11Device* device, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
